...
<!-- 2. 그 값이 없다면 임의의 수를 만들어 세션에 지정 처음한번만 실행되겠지? -->
...
<c:choose>
	<c:when test="${guess lt answer}">작습니다 크게입력하세요</c:when>
	<c:when test="${guess gt answer}">큽니다 작게입력하세요</c:when>
	<c:otherwise>정답</c:otherwise>
</c:choose>
